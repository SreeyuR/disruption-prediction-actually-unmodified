...
def plot_disruptivity_autoformers(
    pred_dict,
):
    """Plot the disruptivity predictions of the Autoformer model.

    Args:
        pred_dict (dict): Dictionary of predictions where "preds"
            is a sequence of predictions, and "label" is whether 
            it disrupted or not.
    ...
    fig, ax = set_up_disruptivity_prediction_plot(
        probs_len=max_len, threshold=0, lims = [-2, 2])  
    
    annotate_label_0 = True
    annotate_label_1 = True
    
    for k, v in pred_dict.items():
        color = "r-" if v["label"] else "g-"
        inds = np.arange(max_len - len(v["preds"]), max_len)
        ax.plot(inds, v["preds"], color, alpha=.5)
        
        # # Annotate the shot if it's from label 0 and hasn't been annotated yet
        # if v["label"] == 0 and annotate_label_0:
        #     annotate_point = (inds[len(inds) // 2], v["preds"][len(v["preds"]) // 2])
        #     ax.annotate(str(k), annotate_point)
        #     annotate_label_0 = False

        # # Annotate the shot if it's from label 1 and hasn't been annotated yet
        # elif v["label"] == 1 and annotate_label_1:
        #     annotate_point = (inds[len(inds) // 2], v["preds"][len(v["preds"]) // 2])
        #     ax.annotate(str(k), annotate_point)
        #     annotate_label_1 = False

    wandb.log({("disruptivity_predictions"): wandb.Image(fig)})
