...
def predict_and_compute_autoformer(
        eval_model,
        dataloader,
        disrupted_index,
        plotting_label,
        ...
        """Predict and compute metrics for the Autoformer model.

        Args:
            eval_model (object): The model to evaluate.
            dataloader (DataLoader): The dataloader to use for evaluation.
            disrupted_index (int): The index of the disrupted column.
            plotting_label (str): The label to use for plotting.
            device (torch.device): The device to use for evaluation.
            stopping_i (int): The number of batches to evaluate. Defaults to None.

        Returns:
            list: The predictions.
            list: The ground truth.
            int: The number of batches evaluated.
        """
        ...
        for i, batch in enumerate(dataloader):
            output_sequences = eval_model.generate(
                ...
                past_observed_mask=batch["attention_mask"].to(device),
                future_time_features=batch["future_time_features"].to(device),
                static_categorical_features=batch["static_categorical_features"].to(device),
            )
            mean_prediction = output_sequences.sequences.mean(dim=1)
            mean_disruption_pred = mean_prediction[:, :, disrupted_index].squeeze()
            mean_disruption_truth = batch["future_values"][:, :, disrupted_index].squeeze()
            
            if len(mean_disruption_pred.shape) == 1:
                mean_disruption_pred = mean_disruption_pred.unsqueeze(0)
                mean_disruption_truth = mean_disruption_truth.unsqueeze(0)

            preds.append(mean_disruption_pred)
            truth.append(mean_disruption_truth)

            if stopping_i is not None and i > stopping_i:
                break

        preds = torch.cat(preds)
        truth = torch.cat(truth)

        evaluation.compute_metrics_autoformer(
            preds, truth, plot_label=plotting_label)
        
        return preds, truth, i
